...
void Fl_RGB_Image::to_rgba(uchar *buf)
{
        int x, y;
        const uchar *old_ptr;
        uchar *new_ptr;
        ...
        new_ptr = buf;
        old_ptr = array;
        for (y = 0; y < h(); ++y, old_ptr = array + y * w() * d() + ld(), new_ptr = buf + y * w() * 4) {
                for (x = 0; x < w(); x++) {
                        uchar p = *(new_ptr++) = *(old_ptr++);
                        if (d() == 1) {
                                *(new_ptr++) = p;
                                *(new_ptr++) = p;
                                *(new_ptr++) = 255; // 255 is plain
                        } else if (d() == 2) {
                                *(new_ptr++) = p;
                                *(new_ptr++) = p;
                                *(new_ptr++) = *(old_ptr++);
                                ...
                                *(new_ptr++) = *(old_ptr++);
                                *(new_ptr++) = *(old_ptr++);
                                *(new_ptr++) = *(old_ptr++); // 0 is transparent
                        }
                }
        }
}
